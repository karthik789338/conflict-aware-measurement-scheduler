...
def run_conflict_factor_analysis(tasks, bin_size, mla, horizon_minutes):
    # build a list of all possible edges between tasks
    task_ids = [t['task_id'] for t in tasks]
    all_possible_edges = list(itertools.combinations(task_ids, 2))
    max_edges = len(all_possible_edges)

    print('\nCONFLICT FACTOR ANALYSIS (bin_size=20, mla from input)')
    print('-' * 72)
    print(f'Tasks        : {", ".join(task_ids)}')
    print(f'Total possible edges : {max_edges}')
    print(f'MLA          : {mla}')
    print()

    # go through each possible edge count from 0 to max
    for edge_count in range(0, max_edges + 1):
        combos = list(itertools.combinations(all_possible_edges, edge_count))

        # run scheduler on every combo and record cycle time + which edges gave it
        # key = cycle time, value = list of edge sets that produced it
        groups = {}
        for combo in combos:
            edges = list(combo)
            cmap = build_conflict_map(tasks, edges)
            h_bins = build_heuristic_bins(tasks, cmap, bin_size, mla)
            ct = len(h_bins) * bin_size
            edge_label = ', '.join(f'{a}-{b}' for a, b in edges) if edges else '(none)'
            if ct not in groups:
                groups[ct] = []
            groups[ct].append(edge_label)

        print(f'Edge count: {edge_count}  ({len(combos)} combination{"s" if len(combos) != 1 else ""})')

        if len(groups) == 1:
            # all combos at this edge count give the same cycle time
            ct = list(groups.keys())[0]
            print(f'  All combinations -> {ct} min cycle time')
            # still list each one so the professor can see them
            for label in groups[ct]:
                print(f'    {label}')
        else:
            # there is variation, split into subgroups
            for ct in sorted(groups.keys()):
                count = len(groups[ct])
                print(f'  -> {ct} min cycle time  ({count} case{"s" if count != 1 else ""})')
                for label in groups[ct]:
                    print(f'    {label}')

        print()

    # build the same data as a dict so main() can save it to json if needed
    data = {'tasks': task_ids, 'mla': mla, 'bin_size': bin_size, 'edge_groups': []}
    for edge_count in range(0, max_edges + 1):
        combos = list(itertools.combinations(all_possible_edges, edge_count))
        groups = {}
        for combo in combos:
            ...
            if ct not in groups:
                groups[ct] = []
            groups[ct].append(edge_label)
        data['edge_groups'].append({
            'edge_count': edge_count,
            'total_combinations': len(combos),
            'cycle_time_groups': [{'cycle_time_min': ct, 'cases': groups[ct]} for ct in sorted(groups.keys())]
        })
...
def main():
    ...
    parser.add_argument('--conflict-analysis', action='store_true', help='Run full conflict factor analysis across all edge combinations')
    ...
    if args.conflict_analysis:
        conflict_data = run_conflict_factor_analysis(tasks, 20, mla, horizon_minutes)
        if args.output:
            conflict_path = args.output.replace('.json', '_conflict.json')
            save_output(conflict_path, conflict_data)
